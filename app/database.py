...
def _migrate_columns():
    ...
    if "users" in insp.get_table_names():
        ...
        if "email_weekly" not in user_cols:
            print("[MIGRATE] Adding email_weekly column to users table...")
            with engine.begin() as conn:
                conn.execute(text(
                    "ALTER TABLE users ADD COLUMN email_weekly BOOLEAN NOT NULL DEFAULT 1"
                ))
            print("[MIGRATE] Done.")
